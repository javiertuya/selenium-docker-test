...
# Nightly build
sudo  \
  NODE_IMAGE="selenium/standalone-chrome:nightly" VIDEO_IMAGE="selenium/video:nightly" \
  FOLDER=$PWD FILE=nightly USER=root docker compose -f compose-test.yml up -d  --quiet-pull
sleep 5
