// copied from https://github.com/javiertuya/selema
...
public class Docker {
	...
	public static void runDocker(String verb, String container) {
		...
		if (!container.equals(dockerOut.trim()))
			throw new RuntimeException(command + " failed. " + dockerOut);
	}
	...
	public static void waitDocker(String container, String expectedLog1, String expectedLog2, int timeoutSeconds) {
		...
		throw new RuntimeException("Container did not become ready in time, last log message was: " + logOut);
	}
	 ...
	 * Returns the container status (exited, running or paused), error message if it does not exist
	 */
	public static String getContainerEnv(String name) {
		return CommandLine.runCommand("docker inspect -f '{{.Config.Env}}' " + name).trim();
	}
